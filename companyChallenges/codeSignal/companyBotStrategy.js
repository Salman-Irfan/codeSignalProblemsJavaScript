        ...
        for (let i in trainingData) {
            if (
                ...
                if (trainingData[i][1] === 1) {
                    correctTrainers.push(trainingData[i][0]);
                }
            }
        }
        // Add a check for empty array
        if (correctTrainers.length === 0) {
            return 0;
        }

        let sumOfCorrectTrainers = correctTrainers.reduce(
            (accumulator, currentValue) => {
                return accumulator + currentValue;
            },
            0
        );
        return sumOfCorrectTrainers / correctTrainers.length; // why it is undefined?
    }
}
